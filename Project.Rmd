...
genre_stats <- titles_ratings %>%
  group_by(genres) %>%
  summarise(avg_rating = mean(averageRating, na.rm = TRUE), count = n()) %>%
  filter(count > 100) %>%
  arrange(desc(avg_rating))

# Join basics with ratings for time analysis
basics_ratings <- titles_basics %>%
  inner_join(ratings, by = "tconst")

# Trend of number of ratings over time
ggplot(basics_ratings, aes(x = startYear, y = numVotes)) +
  geom_point(alpha = 0.1) +
  scale_y_log10() +
  geom_smooth(method = "gam")

# Join names, ratings, and principals
big_join <- principals %>%
  inner_join(ratings, by = "tconst") %>%
...
top_10_people <- big_join %>%
  ...
  summarise(median_rating = median(averageRating), n = n()) %>%
  filter(n >= 5) %>%
...
top_known_for <- known_for_tall %>%
  group_by(primaryName) %>%
  summarise(med_rating = median(averageRating)) %>%
  slice_max(med_rating, n = 1)
'''
